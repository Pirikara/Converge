...
async function resolveTerraform(
  gh: GitHubClient,
  ref: RepoRef,
  base: string,
  candidate: UpdateCandidate,
): Promise<CandidateResolution> {
  // No lockfile / resolution step — just rewrite the constraint string.
  const file = await gh.getFile(ref, candidate.manifestPath, base);
  if (!file) throw new Error(`${candidate.manifestPath} not found`);
  try {
    const edited = editTerraformVersion(file.content, candidate.name, candidate.currentRange, candidate.latestVersion);
    return {
      candidate,
      status: "resolved",
      changes: [{ name: candidate.name, fromRange: candidate.currentRange, toRange: candidate.latestVersion, cobump: false }],
...
async function resolveNuget(
  gh: GitHubClient,
  ref: RepoRef,
  base: string,
  candidate: UpdateCandidate,
): Promise<CandidateResolution> {
  // No lockfile / resolution step — just rewrite the PackageReference version.
  const file = await gh.getFile(ref, candidate.manifestPath, base);
  if (!file) throw new Error(`${candidate.manifestPath} not found`);
  try {
    const edited = editPackageReference(file.content, candidate.name, candidate.currentRange, candidate.latestVersion);
    return {
      candidate,
      status: "resolved",
      changes: [{ name: candidate.name, fromRange: candidate.currentRange, toRange: candidate.latestVersion, cobump: false }],
...
async function resolveHelm(
  gh: GitHubClient,
  ref: RepoRef,
  base: string,
  candidate: UpdateCandidate,
): Promise<CandidateResolution> {
  // No lockfile regen (helm not run) — rewrite the dependency version in Chart.yaml.
  const file = await gh.getFile(ref, candidate.manifestPath, base);
  if (!file) throw new Error(`${candidate.manifestPath} not found`);
  try {
    const edited = editChartVersion(file.content, candidate.name, candidate.currentRange, candidate.latestVersion);
    return {
      candidate,
      status: "resolved",
      changes: [{ name: candidate.name, fromRange: candidate.currentRange, toRange: candidate.latestVersion, cobump: false }],
